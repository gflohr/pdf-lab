...
describe('variations', () => {
	describe('Skia', () => {
		...
		it.skipIf(!hasSkiaFont)('recomputes cbox and advance width', () => {
			...
			expect(Math.round(glyph.cbox.minX * 100) / 100).toBe(179);
		});
	});
	...
	describe('truetype variations', () => {
		...
		it('should use the HVAR table when available for variation metrics', () => {
			...
			expect(
				Math.round(
					font.getVariation({ wght: 150 }).glyphsForString('O')[0]!
						.advanceWidth,
				),
			).toBe(706);
		});
		...
		it('should fall back to the last entry in an HVAR table', () => {
			...
			expect(
				Math.round(
					font.getVariation({ wght: 400 }).glyphsForString('A')[0]!
						.advanceWidth,
				),
			).toBe(584);
		});
		...
		it('should support adjusting GPOS mark anchor points for variations', () => {
			const font = fontkit.openSync(`${datadir}/Mada/Mada-VF.ttf`, {
				wght: 900,
			});
